...
export function AiQueryTab() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const query = await generateSparqlQuery(prompt.trim());
      if (query) {
  ...
  }, [prompt, isConfigured, clearError, generateSparqlQuery]);
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Textarea
              ...
              placeholder="e.g., Find nanopubs about crabs or crab species"
          ...
          {llmError && (
            <div className="rounded-md border border-destructive/50 bg-destructive/10 p-4">
              <div className="text-sm text-destructive">{llmError}</div>
            </div>
          )}
        </div>
      )}

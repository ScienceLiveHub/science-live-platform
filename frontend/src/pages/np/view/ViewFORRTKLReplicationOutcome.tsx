/**
 * ViewFORRTKLReplicationOutcome
 *
 * User-friendly view for nanopubs created with the
 * "FORRT Replication Outcome with Knowledge Loom evidence" template.
 *
 * Extends ViewFORRTReplicationOutcome with Knowledge Loom fields:
 * machine-readable proof, analysis type, and key result.
 */
...
const HAS_MACHINE_READABLE_PROOF =
  "https://w3id.org/sciencelive/o/terms/hasMachineReadableProof";
const HAS_ANALYSIS_TYPE =
  "https://w3id.org/sciencelive/o/terms/hasAnalysisType";
const HAS_KEY_RESULT = "https://w3id.org/sciencelive/o/terms/hasKeyResult";

interface FORRTKLOutcomeData {
  outcomeUri: string;
  label?: string;
  studyUri?: string;
  repoUri?: string;
  ...
  proofUri?: string;
  analysisType?: { uri: string; label?: string };
  keyResult?: string;
}

function extractData(store: NanopubStore): FORRTKLOutcomeData | null {
  ...
  // Knowledge Loom fields
  const proofUri = store.matchOne(
    s,
    namedNode(HAS_MACHINE_READABLE_PROOF),
    null,
    g,
  )?.object.value;

  const analysisTypeUri = store.matchOne(
    s,
    namedNode(HAS_ANALYSIS_TYPE),
    null,
    g,
  )?.object.value;
  const analysisType = analysisTypeUri
    ? {
        uri: analysisTypeUri,
        label: store.findInternalLabel(analysisTypeUri),
      }
    : undefined;

  const keyResult = store.matchOne(s, namedNode(HAS_KEY_RESULT), null, g)
    ?.object.value;
...
export function ViewFORRTKLReplicationOutcome({ store }: CustomViewerProps) {
        ...
        {hasKLData && (
          <div className="rounded-md border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/20">
            ...
            <div className="space-y-2 text-sm">
              ...
              {data.proofUri && (
                <div>
                  <span className="font-medium">Machine-readable proof:</span>{" "}
                  <ExternalUriLink uri={data.proofUri} />
                </div>
              )}
            </div>
          </div>

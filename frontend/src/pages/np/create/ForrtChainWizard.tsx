...
/**
 * A chain draft is fetched from a URL the user supplies, so it is untrusted
 * input: validate it before rendering rather than trusting the JSON's shape.
 * Only the fields this wizard actually reads are required — the draft's
 * descriptive metadata (`schema_version`, `kind`, `template_key`, `provenance`,
 * …) is optional so a slightly older or newer generator still loads.
 */
const chainDraftStepSchema = z.object({
  step: z.string(),
  template_uri: z.string(),
  prefill: z.record(z.string(), z.unknown()).default({}),
  manual: z.array(z.string()).optional(),
  published_uri: z.string().nullish(),
});
...
const carryForwardSchema = z.object({
  ...
  mode: z.enum(["uriList", "uriObjectList"]).optional(),
  itemKey: z.string().optional(),
});

const chainDraftSchema = z.object({
  steps: z.array(chainDraftStepSchema).min(1),
  carry_forward: z.array(carryForwardSchema).default([]),
});

type ChainDraft = z.infer<typeof chainDraftSchema>;
...
export default function ForrtChainWizard() {
  ...
  useEffect(() => {
    ...
    ky.get(corsSafeUrl(draftUrl), { cache: "no-store" })
      ...
      .then((raw) => {
        if (cancelled) return;
        const parsed = chainDraftSchema.safeParse(raw);
        if (!parsed.success) {
          // A well-formed JSON document that isn't a chain draft (or is missing
          // a field we render) must not reach the UI — it would throw mid-render
          // and blank the page. Point at the offending field instead.
          ...
          setDraft(null);
          setError(
            `That file is not a valid chain draft (${where}: ${issue.message}). It should be a chain-draft.json generated by your replication repository.`,
          );
          return;
        }
        const d = parsed.data;
            ...
            {step.manual && step.manual.length > 0 && (
              <>
                {(() => {
                  ...
                  const choose = step.manual.filter(
                    (m) => !(m in step.prefill),
                  );
                  return (
                    <>
